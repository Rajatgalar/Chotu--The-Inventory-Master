...
class PdfCreditNote {
    ...
    fun createUserTable(
        context: Context,
        invoice: CreditNoteModel,
        onFinish: (file: File) -> Unit,
        onError: (Exception) -> Unit
    ) {
        val file = PdfHelper.createFile(context, "credit_note_${invoice.creditNoteDate}_${invoice.creditNoteNumber}")
        ...
        //Add Supplier Dat
        val table = addHeader(invoice.supplierDetail!!,invoice.creditNoteNumber!!, invoice.creditNoteDate!!)
        document.add(table)

        PdfHelper.addLineSpace(document, 1)

        val para = Paragraph("Credit Note", TITLE_FONT)
