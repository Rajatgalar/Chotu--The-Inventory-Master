...
class PdfInvoice {
    ...
    fun createUserTable(
        ...
        val file = PdfHelper.createFile(context, "invoice_${invoice.invoiceDate}_${invoice.invoiceNumber}")
        ...
        val para = Paragraph(if (invoice.invoiceType == "1") "Tax Invoice" else "Bill of Supply", TITLE_FONT)
    ...
    private fun addHeader(supplierDetail: SupplierModel, invoiceNumber: String, invoiceDate: String): PdfPTable {
        ...
        val paragraph = Paragraph( "${supplierDetail.firmName}" +
                (if (!supplierDetail.address.isNullOrBlank()) "\n${supplierDetail.address}" else "") +
                (if (!supplierDetail.city.isNullOrBlank()) "\n${supplierDetail.city}," else "") +
                (if (!supplierDetail.state.isNullOrBlank()) " ${supplierDetail.state}" else "") +
                (if (!supplierDetail.pincode.isNullOrBlank()) " ${supplierDetail.pincode}" else "") +
                (if (!supplierDetail.mobile.isNullOrBlank()) "\n${supplierDetail.mobile}," else "") +
                (if (!supplierDetail.email.isNullOrBlank()) "\n${supplierDetail.email}" else "") +
                (if (!supplierDetail.gstin.isNullOrBlank()) "\n${supplierDetail.gstin}" else ""))
