...
class PdfQuotation {
    ...
    fun createUserTable(
        ...
        val file = PdfHelper.createFile(context, "quotation_${invoice.invoiceDate}_${invoice.invoiceNumber}")
        ...
        val para = Paragraph("Quotation",TITLE_FONT)
        para.alignment = Element.ALIGN_CENTER
        document.add(para)

        PdfHelper.addLineSpace(document, 1)

        val sellerDetailTable = PdfHelper.addBuyerConsigneeAndContactPerson(invoice.buyerDetail!!,
            invoice.consigneeDetailType!!,invoice.consigneeModel!!,invoice.contactDetails!!,document)
    ...
    private fun addHeader(supplierDetail: SupplierModel, invoiceNumber: String, invoiceDate: String): PdfPTable {
        ...
        val paragraph2 = Paragraph("Quotation Number : #$invoiceNumber \nQuotation Date : $invoiceDate")
